...
const saveDraftWorkersComp = async (
  formState: WorkersCompDraftPayload
): Promise<any> => {
  const { data } = await axios.post(
    // $FlowFixMe athleteId cannot be null here as validation will have caught it
    `/athletes/${formState.athlete_id}/workers_comps/save_draft`,
    {
      ...formState,
    }
  );
...
export default saveDraftWorkersComp;
