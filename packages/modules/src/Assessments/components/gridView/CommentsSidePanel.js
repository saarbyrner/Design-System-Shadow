...
import type { CommentsViewType, Athlete, Comments, Comment } from '../../types';
...
const CommentsSidePanel = (props: I18nProps<Props>) => {
  ...
  const isAthletesNavigationAllowed =
    props.viewType === 'PRESENTATION' && props.athletes.length > 2;
  ...
  const onChangeComment = (contentComment: string, currentComment: Comment) => {
    ...
    const newComments = comments.map((comment) =>
      comment.assessmentItemId === currentComment.assessmentItemId
        ? { ...comment, note: { content: contentComment, edit_history: null } }
        : comment
    );
  ...
  const getCommentHistoryDetails = (noteHistory) => {
    const commentDate = getFormattedDate(
      noteHistory.date,
      timezone.orgTimezone
    );
    const commentAuthor = noteHistory.user.fullname;
    return `${commentDate} by ${commentAuthor}`;
  };

  const getCommentDate = () =>
    getFormattedDate(moment(new Date()), timezone.orgTimezone);
              ...
              content={
                <div className="commentsSidePanel__commentBody">
                  <div className="commentsSidePanel__commentDetail">
                    <p>
                      {comment.note?.edit_history
                        ? getCommentHistoryDetails(comment.note.edit_history)
                        : getCommentDate()}
                    </p>
                    {props.isCurrentSquad && (
                      <TextButton
                        onClick={() => props.onChangeViewType('EDITION')}
                        kitmanDesignSystem
                        type="link"
                        text={props.t('Edit')}
                      />
      ...
      >
        <div className="commentsSidePanel__content">
          ...
          <div className="commentsSidePanel__commentsHeader">
            ...
            <TextButton
              ...
              isDisabled={
                areEmptyAllComments(props.comments) &&
                props.viewType === 'PRESENTATION'
              }
            />
          </div>
          <div className="commentsSidePanel__comments">
            {props.viewType === 'EDITION' ? (
        ...
        <div
          className={classnames('slidingPanelActions', {
            'slidingPanelActions--edition': props.viewType === 'EDITION',
          })}
        >
          {props.viewType === 'EDITION' && (
            <TextButton
              onClick={() => {
                setComments(props.comments);
                props.onChangeViewType('PRESENTATION');
              }}
              type="secondary"
              kitmanDesignSystem
              text={props.t('Cancel')}
          ...
          <TextButton
            onClick={() => {
              if (props.viewType === 'PRESENTATION') {
                props.onClose();
              } else {
            ...
            isDisabled={props.viewType === 'EDITION' && !areEditedComments}
          />
        </div>
      </SlidingPanel>
    </div>
