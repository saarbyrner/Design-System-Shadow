...
describe('<Actions />', () => {
  ...
  describe('coachesReportV2', () => {
    ...
    it('Copy last report <Button/> calls the correct endpoint and rehydrates grid when clicked when canCreateNotes is TRUE', async () => {
      // canCreateNotes prop = medical.notes.canCreate permission
      const getMultipleCoachesNotes = jest.spyOn(axios, 'post');
      ...
      expect(copyLastReportButton).toBeInTheDocument();
      ...
      expect(props.rehydrateGrid).toHaveBeenCalledTimes(1);
    });
  });
