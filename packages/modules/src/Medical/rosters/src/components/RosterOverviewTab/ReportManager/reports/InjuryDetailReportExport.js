...
import { exportInjuryDetailReport } from '@kitman/services';
import {
  Box,
  Typography,
  Accordion,
...
const InjuryDetailReportExport = ({
  squadId,
  isSettingsOpen,
  closeSettings,
  reportSettingsKey,
  ...
  const { data: columns = [] }: { data: ReportColumn[] } =
    useGetReportColumnsQuery('injury_detail', { skip: !isSettingsOpen });
  ...
  const reportTitle = t('Injury Detail Report');
  ...
  const onSave = (state) => {
    ...
    let resolvedStatus;
    if (state.injuryStatus === 'resolved') {
      resolvedStatus = true;
    } else if (state.injuryStatus === 'active') {
      resolvedStatus = false;
    }

    let coding;
    let coding_system_pathologies;

    if (state.pathologies?.length > 0 || state.bodyParts?.length > 0) {
      if (isV2MultiCodingSystem) {
        coding_system_pathologies = {};
        if (state.pathologies?.length > 0) {
          coding_system_pathologies.codes = state.pathologies.map(
            ({ id }) => id
          );
        }
        if (state.bodyParts?.length > 0) {
          coding_system_pathologies.body_area_ids = state.bodyParts.map(
            ({ id }) => id
          );
        }
      } else {
        ...
        if (state.pathologies?.length > 0) {
          coding.clinical_impressions.codes = state.pathologies.map(
            ({ id }) => id
          );
        }
        if (state.bodyParts?.length > 0) {
          coding.clinical_impressions.body_area_ids = state.bodyParts.map(
            ({ id }) => id
          );
        }
      }
  ...
  return (
    <>
      ...
      >
        ...
        {isV2MultiCodingSystem ? (
          <>
            <ExportSettings.CommonFields.Mui.PathologiesMultiCodingV2
          ...
          <>
            <ExportSettings.CommonFields.Mui.CIPathologies
              fieldKey="pathologies"
              label={t('CI Code')}
              multiple
              shouldResetValueOnClose
...
export const InjuryDetailReportExportTranslated: ComponentType<Props> =
  withNamespaces()(InjuryDetailReportExport);
export default InjuryDetailReportExport;
