...
const ChronicIssuesTable = (props: I18nProps<Props>) => {
  const columns = [
    {
      id: 'occurrenceDate',
  ...
  return (
    <div css={styles.table}>
      <h3 css={styles.tableTitle}>{props.t('Chronic conditions')}</h3>
      <DataGrid
        columns={columns}
        rows={rows}
        isTableEmpty={props.issues.length === 0}
        emptyTableText={props.t('No chronic condition added')}
        scrollOnBody
      />
    </div>
  );
...
export const ChronicIssuesTableTranslated =
  withNamespaces()(ChronicIssuesTable);
export default ChronicIssuesTable;
