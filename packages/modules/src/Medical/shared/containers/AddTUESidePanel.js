...
import { AddTUESidePanelTranslated as AddTUESidePanel } from '../components/AddTUESidePanel';
...
const AddTUESidePanelContainer = (props) => {
  const dispatch = useDispatch();
  const isOpen = useSelector((state) => state.addTUESidePanel.isOpen);
  const isAthleteSelectable = useSelector(
    (state) => state.addTUESidePanel.initialInfo.isAthleteSelectable
  );
  ...
  return (
    <AddTUESidePanel
      ...
      onClose={() => dispatch(closeAddTUESidePanel())}
...
export default AddTUESidePanelContainer;
