...
import { data } from '@kitman/modules/src/shared/MassUpload/services/mocks/data/mock_staff_csv';
import useStaffUploadGrid from '../useStaffUploadGrid';
...
describe('useStaffUploadGrid', () => {
    ...
    it('returns initial data', async () => {
      await act(async () => {
        renderHookResult = renderHook(
          () => useStaffUploadGrid({ parsedCsv: data.validData }),
          {
            wrapper,
          }
        ).result;
  ...
  describe('[computed data]', () => {
    ...
    it('correctly parses the csv data', async () => {
      await act(async () => {
        renderHookResult = renderHook(
          () => useStaffUploadGrid({ parsedCsv: testData }),
          {
            wrapper,
          }
        ).result;
    ...
    it('exposes the correct values when invalid', async () => {
      await act(async () => {
        renderHookResult = renderHook(
          () => useStaffUploadGrid({ parsedCsv: data.invalidData }),
          {
            wrapper,
          }
        ).result;
    ...
    it('exposes the correct values when invalid, when error is not on last row', async () => {
      ...
      await act(async () => {
        renderHookResult = renderHook(
          () =>
            useStaffUploadGrid({ parsedCsv: testDataWithErrorNotOnLastRow }),
          {
            wrapper,
          }
        ).result;
