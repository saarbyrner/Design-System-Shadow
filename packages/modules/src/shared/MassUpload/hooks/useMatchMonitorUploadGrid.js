...
import matchMonitorHeaders from '../utils/massUploadHeaders/matchMonitorHeaders';

import type {
  CSVMatchMonitor,
  MatchMonitorValidationState,
  RuleSet,
} from '../types';
...
const gridId = 'MatchMonitorUploadGrid';
...
const buttonText = i18n.t('Upload Match Monitors');
const title = i18n.t('Upload Match Monitors');

const useMatchMonitorUploadGrid = ({
  parsedCsv,
}: {
  parsedCsv: Array<CSVMatchMonitor>,
}): ReturnType => {
  ...
  const columns: Array<Column> = useMemo(() => matchMonitorHeaders, []);
  ...
  const ruleset: { [key: string]: RuleSet } = {
    ...getCommonRulesetFields('match_monitor'),
    SquadName: {
      description: i18n.t(
        'The name of the team the match monitor member is part of.'
      ),
      exampleText: 'U16',
      exampleList: [...new Set(squadOptions?.data?.map((squad) => squad.name))],
    },
    Language: {
      description: i18n.t(`The match monitor's language.`),
      exampleText: 'en',
  ...
  const preValidationCheck = (
    matchMonitor: CSVMatchMonitor
  ): MatchMonitorValidationState => {
    return Object.fromEntries(
      Object.keys(matchMonitor).map((key) => {
        if (!validation[key]) {
        ...
        return [key, validation[key](matchMonitor[key])];
      })
    );
  };

  const buildCellData = (matchMonitorMember: CSVMatchMonitor) => {
    const validationCheck = preValidationCheck(matchMonitorMember);
  ...
  const buildRowData = (matchMonitor): Array<Row> => {
    setIsMalformedData(false);
    return matchMonitor.map(buildCellData);
  };
...
export default useMatchMonitorUploadGrid;
