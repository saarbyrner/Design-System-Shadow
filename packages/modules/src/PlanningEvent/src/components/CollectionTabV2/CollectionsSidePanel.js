...
type Props = {
  ...
  setIsSessionEvaluation: Function,
  setIsSessionObjectives: (boolean) => void,
...
const CollectionsSidePanel = (props: I18nProps<Props>) => {
  ...
  const evaluation = props.t('{{eventType}} evaluation', {
    eventType: EVENT_TYPES[props.event.type],
  });
  const objectives = props.t('{{eventType}} objectives', {
    eventType: EVENT_TYPES[props.event.type],
  });

  return (
    <div className="collectionSidePanel" css={style.collectionSidePanel}>
      <SlidingPanel
        align="left"
        cssTop={0}
        isOpen={props.isOpen}
      ...
      >
        ...
        <ul
          data-testid="collectionSidePanel"
          className="collectionSidePanel__list"
        >
          <li
            className="collectionSidePanel__item"
            onClick={() => {
              props.setIsSessionObjectives(false);
              props.setIsCollectionsPanelOpen(false);
              props.setIsSessionEvaluation(true);
            }}
          >
            {evaluation}
          </li>
          <li
            className="collectionSidePanel__item"
            onClick={() => {
              props.setIsSessionEvaluation(false);
              props.setIsCollectionsPanelOpen(false);
              props.setIsSessionObjectives(true);
            }}
          >
            {objectives}
          </li>
