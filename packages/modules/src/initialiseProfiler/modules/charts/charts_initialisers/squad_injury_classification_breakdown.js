...
export default () => {
  const squadInjuryClassificationBreakdownChart = document.getElementById(
    'squad_injury_classification_breakdown_chart'
  );

  if (squadInjuryClassificationBreakdownChart) {
    const data = JSON.parse(
      squadInjuryClassificationBreakdownChart.dataset.chartData
    );
    const categories = JSON.parse(
      squadInjuryClassificationBreakdownChart.dataset.categories
    );
    ...
    const chart_classification_breakdown = new Highcharts.Chart({
      lang: {
        noData: i18n.t('No available data for this date range'),
      },
      chart: {
        renderTo: 'chart_classification_breakdown',
