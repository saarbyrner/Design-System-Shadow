...
import { data } from '@kitman/modules/src/shared/MassUpload/services/mocks/data/mock_growth_and_maturation_csv';
import AppRoot from '@kitman/modules/src/AppRoot';
import useGrowthAndMaturationUploadGrid from '../useGrowthAndMaturationUploadGrid';

describe('useGrowthAndMaturationUploadGrid', () => {
  const wrapper = AppRoot;

  describe('[initial data]', () => {
    it('returns initial data', () => {
      const renderHookResult = renderHook(
        () => useGrowthAndMaturationUploadGrid({ parsedCsv: data.validData }),
        { wrapper }
      ).result;
  ...
  describe('[computed data]', () => {
    ...
    it('correctly parses the csv data', () => {
      renderHookResult = renderHook(
        () => useGrowthAndMaturationUploadGrid({ parsedCsv: testData }),
        { wrapper }
      ).result;
    ...
    it('exposes the correct values when invalid, when error is not on last row', async () => {
      ...
      renderHookResult = renderHook(
        () =>
          useGrowthAndMaturationUploadGrid({
  ...
  describe('[isError]', () => {
    ...
    it('exposes the correct value when isError', () => {
      renderHookResult = renderHook(
        () => useGrowthAndMaturationUploadGrid({ parsedCsv: data.invalidData }),
        { wrapper }
      ).result;
