...
import { Checkbox, TooltipMenu, TextButton } from '@kitman/components';
...
type OnRowChange = (id: number) => void;
...
const DescriptionFormatter = ({ row }: { row: Row }) => {
  ...
  return <span css={style.description} ref={ref} />;
};
...
export default ({
  ...
  onArchive: (id: number) => Promise<typeof undefined>,
}): Array<HeaderData> => [
  {
    key: 'checkbox',
    name: '',
  ...
  },
  {
    key: 'tooltip-menu',
    name: '',
    formatter: ({ row: { id } }: { row: Row }) => (
      <div css={style.gridTooltipMenu}>
        <div css={style.tooltipMenu} data-testid={`${id}-drill-tooltip-menu`}>
          <TooltipMenu
            placement="bottom-end"
            menuItems={[
              {
                description: t('Archive drill'),
