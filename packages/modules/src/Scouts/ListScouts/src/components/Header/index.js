    ...
    <ProfileHeaderLayout>
      <ProfileHeaderLayout.Main>
        <ProfileHeaderLayout.Content>
          <Fragment>
            <h2 css={style.title}>{props.t('Manage Scouts')}</h2>
          </Fragment>
        </ProfileHeaderLayout.Content>
        ...
        <ProfileHeaderLayout.Actions>
          <TextButton
            type="primary"
            text={props.t('Create New Scout')}
            onClick={() => {
              locationAssign(`/administration/scouts/new`);
            }}
            kitmanDesignSystem
          />
          {window.featureFlags['league-ops-mass-create-athlete-staff'] &&
            permissions?.settings?.canCreateImports && (
              <>
                <MassUpload userType="scout" />
                <DownloadCSV userType="scout" />
              </>
            )}
        </ProfileHeaderLayout.Actions>
      </ProfileHeaderLayout.Main>
